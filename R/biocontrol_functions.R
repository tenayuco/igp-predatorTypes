...
biocontrol_databaser <- function(
  ...
  DF_BIOCON_N$IpCat[
    DF_BIOCON_N$normIp %in% sort(unique(DF_BIOCON_N$normIp))[1:2]
  ] <- "IpMin" ##agarara los dos valoes menores y sobre eso los catwegotri como los inimos (el de pb y el hv)
  DF_BIOCON_N$IpCat[
    DF_BIOCON_N$normIp %in% sort(unique(DF_BIOCON_N$normIp))[3]
  ] <- "IpMax" ##agarara los dos valoes menores y sobre eso los catwegotri como los inimos (el de pb y el hv)

  DF_BIOCON_N$InCat[
    DF_BIOCON_N$normIn %in% sort(unique(DF_BIOCON_N$normIn))[1:2]
  ] <- "InMin"
  DF_BIOCON_N$InCat[
  ...
  DF_BIO <- DF_BIO |>
    dplyr::mutate(
      s_cat = ifelse(
        S > 0.66,
        "competitive",
        ifelse(S > 0.33, "symmetric", "tritrophic")
      )
    ) |>
    dplyr::select(
      Ip,
...
plotter_absolute <- function(
  da_ta_coex,
  var_plot,
  ip_values = c("IpMin", "IpMax"),
  in_values = c("InMin", "InMax")
) {
  if (!(var_plot %in% c("meanR", "meanP", "meanN"))) {
    print("error: var_plot shoud be meanR, meanP or meanN")
    return()
  } else {
    ...
    for (Ip_chosen in ip_values) {
      for (In_chosen in in_values) {
        BIO_PLOT <- DF_PLOT |>
          ...
          scale_shape_manual(
            values = c("competitive" = 22, "symmetric" = 21, "tritrophic" = 24)
          ) +
        ...
        file_subfolder <- paste0(
          "sval_",
          paste0(s_val, collapse = "_"),
          "K_",
          paste0(k_val, collapse = "_"),
          "/"
        )
        file_name <- paste0(
          "PLOT_BIOCONTROL_",
...
#################now the differencer for the table

##############33here is a simplified converter. That should results in the same

differencer_cat_lblb_treatment <- function(
  da_ta_coex,
  ip_chosen = c("IpMin", "IpMax"),
  in_chosen = c("InMin", "InMax")
) {
  ...
  DF$combPred_cat[
    (DF$IGP_P %in% c("PB", "HV")) & (DF$IGP_N %in% c("PB", "HV"))
  ] <- "Changing both"
  ...
  DF_CHANGE <- DF_CHANGE |>
    dplyr::mutate(
      ...
      mean_coexi_change1 = 100 *
        (meanCoexistence.x - meanCoexistence.y) /
        (meanCoexistence.x + meanCoexistence.y),
  ...
  #########
  file_folder <- "./data/biocontrol/"
  file_subfolder <- "summarized_table_cat/"

  file_name <- paste0("DF_CHANGE_LBLB", ".csv")

  ##this is a control AFTER RUNNING THE ANALYSIS, but it is not really important

  if (file.exists(paste0(file_folder, file_subfolder, file_name))) {
    print("DF_CHANGE_LBLB exists already")
  } else {
    dir.create(paste0(file_folder, file_subfolder), recursive = TRUE)

    write_csv(DF_CHANGE, paste0(file_folder, file_subfolder, file_name))
  }
...
differencer_lblb <- function(da_ta_coex, criteria_change) {
  ...
  DF_CHANGE$meanR_rel_change_cat[
    DF_CHANGE$meanR_rel_change > criteria_change
  ] <- 1 ### the 10 percet is
  DF_CHANGE$meanR_rel_change_cat[
    DF_CHANGE$meanR_rel_change < criteria_change * (-1)
  ] <- -1 ### the 10 percet is
...
plotter_relative_lblb <- function(
  da_ta_rel,
  var_plot,
  ip_values = c("IpMin", "IpMax"),
  in_values = c("InMin", "InMax")
  ...
  if (
    ...
    print(
      "error: var_plot shoud be meanR_change, meanP_change or meanN_change, meanR_rel_change, meanP_rel_change or meanN_rel_change"
    )
    return()
  } else {
    ...
    named_var_change <- c(
      "meanP_change" = "Change in IG predator P",
      "meanN_change" = "Change in IG prey N",
      "meanR_change" = "Change in Herbivore H"
    )
    ...
    for (Ip_chosen in ip_values) {
      for (In_chosen in in_values) {
        BIO_PLOT_CHANGE <- DF_PLOT |>
          ...
          geom_line(aes(linetype = s_cat), linewidth = 0.5, color = "black") +
          geom_segment(
            aes(x = 0, y = 0, xend = 8, yend = 0),
            color = "darkred",
            linetype = 1
          ) +
          geom_point(
            aes(fill = as.factor(coexi_change), shape = s_cat),
            size = 2.5,
            alpha = 0.8
          ) + ###opcion1
          # geom_point(aes(fill=as.character(coexistence), shape = s_cat), size=2, alpha=1) +  ##opcion 2
          ...
          scale_fill_manual(
            values = c("1" = "darkblue", "0" = "white", "-1" = "darkred")
          ) + ##opcion 1
          #scale_fill_manual(values = s_cat_col) +  #opcion2
          ...
          labs(
            ...
            y = paste0(
              "Population density of the ",
              named_var_change[plotted_var_change],
              " at equilibrium"
            ),
            fill = "IGP module",
            shape = "IGP symmetry (s)",
        ...
        ggsave(
          BIO_PLOT_CHANGE,
          filename = paste0(
            "./output/numericalIGP/biocontrol/change/",
            plotted_var_change,
            "_scat_allK_S=",
...
summariser_categories <- function(
  da_ta_rel,
  ip_values = c("IpMin", "IpMax"),
  in_values = c("InMin", "InMax")
) {
  ...
  for (Ip_chosen in ip_values) {
    for (In_chosen in in_values) {
      DF_SUM <- DF |>
        ...
        dplyr::summarise(
          total_meanR_cat = mean(meanR_rel_change_cat),
      ...
      DF_SUM$total_coexistence_cat[
        DF_SUM$total_coexistence_cat <= 0.5 * (-1)
      ] <- -1
...
plotter_relative_lblb_cat <- function(
  da_ta_cat,
  ip_values = c("IpMin", "IpMax"),
  in_values = c("InMin", "InMax")
) {
  ...
  for (Ip_chosen in ip_values) {
    for (In_chosen in in_values) {
      BIO_CAT_CHANGE <- DF_PLOT |>
        ...
        geom_rect(
          aes(xmin = 0, xmax = 8, ymin = 0, ymax = 1),
          fill = "darkgreen",
          alpha = 0.1
        ) +
        geom_line() +
        geom_point(
          aes(fill = as.factor(total_coexistence_cat)),
          shape = 21,
          size = 3
        ) +
        ...
        scale_fill_manual(
          values = c("1" = "black", "0" = "white", "-1" = "white")
        ) +
      ...
      ggsave(
        BIO_CAT_CHANGE,
        filename = paste0(
          "./output/numericalIGP/biocontrol/change_cat/",
          "biocat_allK_S=",
          paste(unique(DF_PLOT$S), collapse = "_"),
          Ip_chosen,
          "_",
