...
DF_BIOCON_N$IpCat[DF_BIOCON_N$normIp %in% sort(unique(DF_BIOCON_N$normIp))[3]] <- "IpMax" ##agarara los dos valoes menores y sobre eso los catwegotri como los inimos (el de pb y el hv)


DF_BIOCON_N$InCat[DF_BIOCON_N$normIn %in% sort(unique(DF_BIOCON_N$normIn))[1:2]] <- "InMin"
DF_BIOCON_N$InCat[DF_BIOCON_N$normIn %in% sort(unique(DF_BIOCON_N$normIn))[3]] <- "InMax"
...
##this is a control AFTER RUNNING THE ANALYSIS, but it is not really important 
  ...
  if(file.exists(paste0(file_folder, file_subfolder, file_name))){
  print("DF_CHANGE_LBLB exists already")} else{
